...
def 每日全省案件处罚数据():
    ...
    sql_交通非现查处数 = {
        ...
        "where": " record_type = 31 AND insert_type = 1 AND data_source = 1 AND case_type = 1 AND close_case_time between '{} 00:00:00' AND  '{} 23:59:59' AND area_province = '330000' GROUP BY  dept_county".format(
            start_time, end_time),
        "columns": "dept_county as area_county,count(DISTINCT ( CASE_NUM )) AS 交通非现查处数,  count(DISTINCT IF( car_no LIKE '%浙%', CASE_NUM, NULL )) AS 交通非现查处数本省 ,SUM(punish_money) 交通非现处罚金额 "
    }
    ...
    交通非现查处数['交通非现处罚金额'] = 交通非现查处数['交通非现处罚金额'].astype('float')
    ...
    U_all = pd.DataFrame(U_all,
                           columns=['地市', '区县', '交通非现查处数', '交通非现处罚金额', '交通现场查处数', '交警现场查处数', 'city_code', 'area_county'])
    ...
    U_all地市 = pd.DataFrame(U_all地市,
                           columns=['地市', '交通非现查处数', '交通非现处罚金额', '交通现场查处数', '交警现场查处数'])
    U_all省=U_all地市.copy()
    U_all省['省']='浙江'
    U_all省 = U_all省.groupby(['省']).sum().reset_index()
    U_all省 = pd.DataFrame(U_all省,
                           columns=['省', '交通非现查处数', '交通非现处罚金额', '交通现场查处数', '交警现场查处数'])

    U_all省.rename(
        columns={'省': '地市'}, inplace=True)
    U_all省=pd.concat([U_all地市,U_all省])
    print(U_all省)
    with pd.ExcelWriter(r'C:\Users\stayhungary\Desktop\每日全省案件处罚数据7.xlsx') as writer1:
    ...
    file_path=r'C:\Users\stayhungary\Desktop\每日全省案件处罚数据7.xlsx'
    write_excel_file(file_path)
    from threading import Timer
    import datetime
    """定时1天"""
    now_time = datetime.datetime.now()
    next_time = now_time + datetime.timedelta(days=+7)
...
def To_emil():
    ...
    smtp_password = 'fuhaoahvxedeffdg'  # 邮箱授权码
    ...
    csv_file = open(r'C:\Users\stayhungary\Desktop\每日全省案件处罚数据7.xlsx', 'rb').read()

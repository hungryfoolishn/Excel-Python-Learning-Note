...
def data_sours_all(c,j,county):
        ...
        sql_pass = {
            "tableName": "t_bas_pass_data_31  ",
            "where": "  area_county ={}  and out_station_time between '{} 00:00:00' and '{} 00:00:00' GROUP BY out_station ".format(
                county,starttime, endtime),
            "columns": "out_station, count(1) as pass_num, sum(is_truck) as truck_num"
        }
        sql_station = {
        ...
        sql={
        "tableName": "t_bas_basic_data_pass",
        "where": " statistic_date= '{}' and county_code ={} ".format(starttime,county),
        "columns": "*"
         }
        delete_data(sql)
...
def chushihua():
    ...
    while c <= 60:
        ...
        area_county['area_county'] = area_county['area_county'].astype('int64')
        area_county = area_county['area_county']
        area_county = list(area_county)
        for area_city in area_county:
            data_sours_all(c,j,area_city)
        c += 1
        j += 1

def pass_num_0(c,j,out_station):
    from datetime import datetime
    day = datetime.now().date()  # 获取当前系统时间
    from datetime import datetime
    import datetime
    ...
    pass_truck_num['truck_num'] = pass_truck_num['truck_num'].astype('int')
    pass_num=pass_truck_num['truck_num']
    print(pass_num)
    return  pass_num.empty
...
def area_countypass_num_0(c,j,area_county):
    ...
    sql_pass = {
        ...
        "columns": "out_station, count(1) as truck_num "
    }

    pass_truck_num = get_data(sql_pass)
    pass_truck_num = pd.DataFrame(pass_truck_num,
                                  columns=['out_station', 'truck_num'])

    pass_truck_num=pass_truck_num.fillna(0)
    pass_truck_num['truck_num'] = pass_truck_num['truck_num'].astype('int')
    pass_num=pass_truck_num['truck_num']
    print(pass_truck_num.head(1))
    return  pass_num.empty


def chushihuastation():
    c = 1
    j = 0
    while c <= 60:
        from datetime import datetime
        ...
        sql_station = {
            ...
            "where": "  a.is_deleted = 0 and a.station_type in (31) and a.status =0  ",
            "columns": " a.station_code "
        }

        out_station = get_data(sql_station)
        out_station = out_station['station_code']
        out_station = list(out_station)
        print(out_station)
        for out_station in out_station:
            if pass_num_0(c,j,out_station) =="True"  :
                continue
            data_sours_allstaton(c,j,out_station)
...
def All_county_code(c):
    ...
    sql = {
        "tableName": "t_bas_basic_data_pass",
        "where": " statistic_date= '{}'  group by county_code ".format(starttime),
        "columns": "county_code"
    }
    county_code = get_data(sql)
...
@retry(max_attempts=30, delay=300)
def chushihua区县():
    c = 1
    j = 0
    while c <= 61:
        ...
        if area_county.empty == 'True' :
            continue
        for area_city in area_county:
            if area_countypass_num_0(c, j, area_city) :
                continue
            data_sours_all(c, j, area_city)
        c += 1
        j += 1

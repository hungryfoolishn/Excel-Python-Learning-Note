...
@pysnooper.snoop()
def data_sours_all(c, j, county):
    print('data_sours_all', county)
    import pandas as pd
    ...
    sql_31 = {
        ...
        "where": "  allow IS NULL and  is_unusual = 0 and area_county ='{}'  and out_station_time between '{} 00:00:00' and '{} 00:00:00'  ".format(
            county, starttime, endtime),
        "columns": "area_city,area_county,out_station,direction,out_station_time,car_no,total_weight,limit_weight"
    }
    sql_71 = {
        "tableName": "t_bas_over_data_71   ",
        "where": "  area_county ={} and out_station_time between '{} 00:00:00' and '{} 00:00:00'  ".format(
            county, starttime, endtime),
        "columns": "area_county,area_county,out_station,out_station_time,car_no,total_weight,limit_weight"
    }
    ...
    sql_pass = {
        ...
        "where": "  area_county ={}  and out_station_time between '{} 00:00:00' and '{} 00:00:00' GROUP BY out_station,direction ".format(
            county, starttime, endtime),
        "columns": "out_station,direction, count(1) as pass_num, sum(is_truck) as truck_num"
    }
    sql_station = {
        "tableName": "t_sys_station  ",
        "where": "  is_deleted = 0   and station_type in (21,31,71) and area_county ={}  ".format(county),
    ...
    pass_truck_num = get_data(sql_pass)
    pass_truck_num = pd.DataFrame(pass_truck_num,
                                      columns=['out_station', 'direction',
                                               'pass_num', 'truck_num'])
    pass_truck_num['truck_num'] = pass_truck_num['truck_num'].astype('int')
    pass_truck_num['direction'] = pass_truck_num['direction'].fillna(1)
    print(pass_truck_num.head())
    ...
    t_bas_pass_data_31 = pd.DataFrame(t_bas_pass_data_31,
                                      columns=['area_city', 'area_county', 'out_station', 'direction',
                                               'out_station_time', 'car_no', 'total_weight', 'limit_weight'])
    t_bas_pass_data_31['direction'] = t_bas_pass_data_31['direction'].fillna(1)
    ...
    t_bas_pass_data_71 = pd.DataFrame(t_bas_pass_data_71,
                                      columns=['area_city', 'area_county', 'out_station', 'out_station_time', 'car_no',
    ...
    U_pass31_area_station = pd.DataFrame(U_pass31_area_station,
                                         columns=['out_station', 'direction', 'out_station_time', 'car_no',
                                                  'total_weight',
                                                  'limit_weight',
                                                  'overrun_rate'])
    ...
    T0_5T = U_pass31_area_station[
        ...
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T0_5T')

    U_pass31_area_station = U_pass31_area_station[(U_pass31_area_station.total_weight >= 2.5)]

    over_num = U_pass31_area_station[(U_pass31_area_station['overrun_rate'] > 0)].groupby(['out_station', 'direction'])[
        'car_no'].count().reset_index(name='over_num')
    wide_table_31 = pd.merge(wide_table_31, over_num, on=['out_station', 'direction'], how='left')
    ...
    U_pass31_area_station['car_no'] = U_pass31_area_station['car_no'].astype('string')
    no_car = \
        U_pass31_area_station[(U_pass31_area_station['car_no'].str.len() <= 5)].groupby(['out_station', 'direction'])[
            'car_no'].count().reset_index(name='no_car')
    wide_table_31 = pd.merge(wide_table_31, no_car, on=['out_station', 'direction'], how='left')

    ##超限程度分布
    C0_10X = U_pass31_area_station[
        ((0 < U_pass31_area_station['overrun_rate']) & (U_pass31_area_station['overrun_rate'] < 10))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='C0_10X')
    C10_20X = U_pass31_area_station[
        ((10 <= U_pass31_area_station['overrun_rate']) & (U_pass31_area_station['overrun_rate'] < 20))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='C10_20X')
    C20_30X = U_pass31_area_station[
        ((20 <= U_pass31_area_station['overrun_rate']) & (U_pass31_area_station['overrun_rate'] < 30))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='C20_30X')
    C30_40X = U_pass31_area_station[
        ((30 <= U_pass31_area_station['overrun_rate']) & (U_pass31_area_station['overrun_rate'] < 40))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='C30_40X')
    C40_50X = U_pass31_area_station[
        ((40 <= U_pass31_area_station['overrun_rate']) & (U_pass31_area_station['overrun_rate'] < 50))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='C40_50X')
    C50_60X = U_pass31_area_station[
        ((50 <= U_pass31_area_station['overrun_rate']) & (U_pass31_area_station['overrun_rate'] < 60))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='C50_60X')
    C60_70X = U_pass31_area_station[
        ((60 <= U_pass31_area_station['overrun_rate']) & (U_pass31_area_station['overrun_rate'] < 70))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='C60_70X')
    C70_80X = U_pass31_area_station[
        ((70 <= U_pass31_area_station['overrun_rate']) & (U_pass31_area_station['overrun_rate'] < 80))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='C70_80X')
    C80_90X = U_pass31_area_station[
        ((80 <= U_pass31_area_station['overrun_rate']) & (U_pass31_area_station['overrun_rate'] < 90))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='C80_90X')
    C90_100X = U_pass31_area_station[
        ((90 <= U_pass31_area_station['overrun_rate']) & (U_pass31_area_station['overrun_rate'] < 100))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='C90_100X')
    C100X = \
        U_pass31_area_station[(100 <= U_pass31_area_station['overrun_rate'])].groupby(['out_station', 'direction'])[
            'car_no'].count().reset_index(name='C100X')

    ##聚合超限程度
    wide_table_31 = pd.merge(wide_table_31, C0_10X, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, C10_20X, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, C20_30X, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, C30_40X, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, C40_50X, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, C50_60X, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, C60_70X, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, C70_80X, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, C80_90X, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, C90_100X, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, C100X, on=['out_station', 'direction'], how='left')

    ##吨位分布
    T0_5T = T0_5T
    T5_10T = U_pass31_area_station[
        ((5 <= U_pass31_area_station['total_weight']) & (U_pass31_area_station['total_weight'] < 10))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T5_10T')
    T10_20T = U_pass31_area_station[
        ((10 <= U_pass31_area_station['total_weight']) & (U_pass31_area_station['total_weight'] < 20))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T10_20T')
    T20_30T = U_pass31_area_station[
        ((20 <= U_pass31_area_station['total_weight']) & (U_pass31_area_station['total_weight'] < 30))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T20_30T')
    T30_40T = U_pass31_area_station[
        ((30 <= U_pass31_area_station['total_weight']) & (U_pass31_area_station['total_weight'] < 40))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T30_40T')
    T40_50T = U_pass31_area_station[
        ((40 <= U_pass31_area_station['total_weight']) & (U_pass31_area_station['total_weight'] < 50))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T40_50T')
    T50_60T = U_pass31_area_station[
        ((50 <= U_pass31_area_station['total_weight']) & (U_pass31_area_station['total_weight'] < 60))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T50_60T')
    T60_70T = U_pass31_area_station[
        ((60 <= U_pass31_area_station['total_weight']) & (U_pass31_area_station['total_weight'] < 70))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T60_70T')
    T70_80T = U_pass31_area_station[
        ((70 <= U_pass31_area_station['total_weight']) & (U_pass31_area_station['total_weight'] < 80))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T70_80T')
    T80_90T = U_pass31_area_station[
        ((80 <= U_pass31_area_station['total_weight']) & (U_pass31_area_station['total_weight'] < 90))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T80_90T')
    T90_100T = U_pass31_area_station[
        ((90 <= U_pass31_area_station['total_weight']) & (U_pass31_area_station['total_weight'] < 100))].groupby(
        ['out_station', 'direction'])['car_no'].count().reset_index(name='T90_100T')
    T100T = \
        U_pass31_area_station[(100 <= U_pass31_area_station['total_weight'])].groupby(['out_station', 'direction'])[
            'car_no'].count().reset_index(name='T100T')

    ##聚合吨位分布
    wide_table_31 = pd.merge(wide_table_31, T0_5T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T5_10T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T10_20T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T20_30T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T30_40T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T40_50T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T50_60T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T60_70T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T70_80T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T80_90T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T90_100T, on=['out_station', 'direction'], how='left')
    wide_table_31 = pd.merge(wide_table_31, T100T, on=['out_station', 'direction'], how='left')
    ...
    wide_table_31 = pd.DataFrame(wide_table_31,
                                 columns=['out_station', 'direction', 'pass_num', 'truck_num',
    ...
    U_pass71_area_station['car_no'] = U_pass71_area_station['car_no'].astype('string')
    no_car = U_pass71_area_station[(U_pass71_area_station['car_no'].str.len() <= 5)].groupby(['out_station'])[
        'car_no'].count().reset_index(name='no_car')
    ...
    wide_table = pd.merge(t_sys_station, wide_table, left_on='station_code', right_on='out_station', how='left')
    ##市际站点区域变更
    市际站点 = pd.read_excel(r'C:\Users\liu.wenjie\Desktop\月报\t_code_area.xlsx', sheet_name='市际站点')
    ...
    wide_table['id'] = wide_table['station_code'].astype('string') + wide_table['direction'].astype('string')+starttime1
    ...
    # wide_table.to_excel(r'C:\Users\liu.wenjie\Desktop\wide_table市际站点2.xlsx')
    '''数据库删除'''
    wide_table1=wide_table['station_code'].astype('str').drop_duplicates()
    column = wide_table1.to_list()
    column_str = str(column).replace('[', "(").replace(']', ")")
    sql = {
        "tableName": "t_bas_basic_data_pass",
        "where": " statistic_date= '{}' and station_code {} ".format(starttime, column_str),
        "columns": "*"
    }
    delete_data(sql)
...
def data_sours_allstaton(c, j, out_station):
    import pandas as pd
    from datetime import datetime
    day = datetime.now().date()  # 获取当前系统时间
    ...
    sql_31 = {
        "tableName": "t_bas_over_data_31   ",
        "where": "  allow IS NULL AND is_unusual = 0 and out_station='{}'  and out_station_time between '{} 00:00:00' and '{} 00:00:00'  ".format(
            out_station, starttime, endtime),
        "columns": "area_city,area_county,out_station,out_station_time,car_no,total_weight,limit_weight"
    }
    # sql_71 = {
    #     "tableName": "t_bas_over_data_71   ",
    ...
    sql_pass = {
        "tableName": "t_bas_pass_data_31  ",
        "where": "  out_station='{}'  and out_station_time between '{} 00:00:00' and '{} 00:00:00' GROUP BY out_station ".format(
            out_station, starttime, endtime),
        "columns": "out_station, count(1) as pass_num, sum(is_truck) as truck_num"
    }
    ...
    sql_station = {
        "tableName": "t_sys_station  ",
        "where": "  is_deleted = 0   and station_code ='{}'  ".format(out_station),
        "columns": "station_code,station_status,station_type,is_check_station,station_name, area_county"
    }
    ...
    sql = {
        "tableName": "t_bas_basic_data_pass",
        "where": " statistic_date= '{}' and station_code ='{}' ".format(starttime, out_station),
        "columns": "*"
    }
    delete_data(sql)
...
def area_countypass_num_0(c, j, area_county):
    from datetime import datetime
    day = datetime.now().date()  # 获取当前系统时间
    from datetime import datetime
    import datetime
    ...
    sql_pass = {
        "tableName": "t_bas_over_data_31  ",
        "where": "  area_county='{}'  and out_station_time between '{} 00:00:00' and '{} 00:00:00'  ".format(
            area_county, starttime, endtime),
        "columns": " count(id) as truck_num "
    }
    ...
    pass_truck_num = pass_truck_num.fillna(0)
    pass_truck_num['truck_num'] = pass_truck_num['truck_num'].astype('int')
    pass_num = pass_truck_num['truck_num'].values
    print(pass_num)
    return pass_num
...
def All_county_code(c):
    from datetime import datetime
    day = datetime.now().date()  # 获取当前系统时间
    import datetime
    starttime = day - datetime.timedelta(days=c)
    area_county = pd.read_excel(r'C:\Users\liu.wenjie\Desktop\月报\t_code_area.xlsx', sheet_name='通报')
    area_county['area_county'] = area_county['area_county']
    All_area_county = area_county['area_county'].astype('string')
    sql = {
        "tableName": "t_bas_basic_data_pass",
        "where": " statistic_date= '{}'  group by station_code ".format(starttime),
        "columns": "station_code"
    }
    station_code = get_data(sql)
    station_code = pd.DataFrame(station_code,
                               columns=['station_code'])

    station = {
        "tableName": "t_sys_station  ",
        "where": "  is_deleted = 0   and station_type in (21,31,71)   ",
        "columns": "station_code, area_county as county_code "
    }
    county_code = get_data(station)
    county_code = pd.DataFrame(county_code,
                               columns=['station_code','county_code'])
    county_code = pd.merge(station_code, county_code, on=['station_code'], how='left')

    county_code = county_code['county_code'].drop_duplicates()
    county_code = list(county_code)
    All_area_county = list(All_area_county)
    area_county = []
    for m in All_area_county:
        if m not in county_code:
            area_county.append(m)
    area_county = pd.Series(area_county)

    return area_county


@retry(max_attempts=30, delay=3)
def 区县():
    c = 2
    j = 1
    while c <= 2:
        ...
        ks = datetime.now()
        print('初始化开始时间', ks)
        area_county = All_county_code(c)

        if area_county.empty == 'True':
            continue
        for area_city in area_county:
            if area_countypass_num_0(c, j, area_city) == 0:
                continue
            data_sours_all(c, j, area_city)
        c += 1
        j += 1
    ...
    next_time2 = datetime.datetime.strptime(
        str(next_year) + "-" + str(next_month) + "-" + str(next_day) + " " + "2:00:00", "%Y-%m-%d %H:%M:%S")
    timer_start_time2 = (next_time2 - now_time).total_seconds()
    print('下次首页运行耗时', next_time2)
    t2 = Timer(timer_start_time2, 区县)  # 此处使用递归调用实现
    t2.start()
...
def t_bas_deal_case_statistics_data(k, j):
    from datetime import datetime
    day = datetime.now().date()  # 获取当前系统时间
    today = datetime.now()
    from datetime import datetime
    ...
    now = datetime.datetime.now()
    start_time1 = datetime.datetime(now.year, now.month, 1).date()
    # start_time= ('{}' + '-01-01').format(year)
    start_time = start_time1 - relativedelta(months=k)
    print('starttime', start_time)
    end_time = start_time1 - relativedelta(months=j)
    ...
    first_trial_pass_num = data2[(data2['status'] != 15)].groupby(['area_code'])['record_code'].count().reset_index(
        name='first_trial_pass_num')
    check_num = data2[(data2['status'] != 15)].groupby(['area_code'])['record_code'].count().reset_index(
        name='check_num')
    check_pass_num = data2[(data2['status'] != 15) & (data2['status'] != 0)].groupby(['area_code'])[
        'record_code'].count().reset_index(
        name='check_pass_num')
    illegal_judgment_num = data2[(data2['law_judgment'] == 1)].groupby(['area_code'])[
        'record_code'].count().reset_index(
    ...
    sms_inform_num = data2[(data2['dx_count'] > 0)].groupby(['area_code'])['record_code'].count().reset_index(
        name='sms_inform_num')
    case_num = data2[(data2['status'] == 13) & (data2['status'] == 6)].groupby(['area_code'])[
        'record_code'].count().reset_index(
        name='case_num')
    overrun100_case_num = data2[(data2['status'] == 13) & (data2['status'] == 6) & (data2['overrun_rate'] >= 100) & (
                data2['total_weight'] < 100)].groupby(['area_code'])['record_code'].count().reset_index(
        name='overrun100_case_num')
    hundred_king_case_num = \
    data2[(data2['status'] == 13) & (data2['status'] == 6) & (data2['total_weight'] >= 100)].groupby(['area_code'])[
        'record_code'].count().reset_index(
        name='hundred_king_case_num')
    case_over_num = data2[(data2['status'] == 13)].groupby(['area_code'])['record_code'].count().reset_index(
        name='case_over_num')
    overrun100_case_over_num = \
    data2[(data2['status'] == 13) & (data2['overrun_rate'] >= 100) & (data2['total_weight'] < 100)].groupby(
        ['area_code'])['record_code'].count().reset_index(
        name='overrun100_case_over_num')
    hundred_king_case_over_num = data2[(data2['status'] == 13) & (data2['total_weight'] >= 100)].groupby(['area_code'])[
        'record_code'].count().reset_index(
    ...
    t_bas_deal_case_statistics_data = pd.merge(t_bas_deal_case_statistics_data, check_pass_num, on=['area_code'],
                                               how='left')
    t_bas_deal_case_statistics_data = pd.merge(t_bas_deal_case_statistics_data, illegal_judgment_num, on=['area_code'],
                                               how='left')
    t_bas_deal_case_statistics_data = pd.merge(t_bas_deal_case_statistics_data, letter_inform_num, on=['area_code'],
                                               how='left')
    t_bas_deal_case_statistics_data = pd.merge(t_bas_deal_case_statistics_data, sms_inform_num, on=['area_code'],
                                               how='left')
    ...
    t_bas_deal_case_statistics_data = pd.merge(t_bas_deal_case_statistics_data, case_over_num, on=['area_code'],
                                               how='left')
    t_bas_deal_case_statistics_data = pd.merge(t_bas_deal_case_statistics_data, overrun100_case_over_num,
                                               on=['area_code'],
                                               how='left')
    t_bas_deal_case_statistics_data = pd.merge(t_bas_deal_case_statistics_data, hundred_king_case_over_num,
                                               on=['area_code'],
                                               how='left')
    ...
    t_bas_deal_case_statistics_data['statistics_date'] = start_time.strftime("%Y-%m")
    print('statistics_date', start_time.strftime("%Y-%m"))
    t_bas_deal_case_statistics_data['first_trial_pass_rate'] = t_bas_deal_case_statistics_data.apply(
        lambda x: x['first_trial_pass_num'] / x['first_trial_num'] * 100, axis=1).round(2)
    ...
    '''数据库删除'''
    sql = {
        "tableName": "t_bas_deal_case_statistics_data",
        "where": " statistics_date= '{}' ".format(start_time.strftime("%Y-%m")),
        "columns": "*"
    }
    delete_data(sql)

    '''数据插入'''
    df = t_bas_deal_case_statistics_data.reset_index(drop=True)
    # df=df.drop(['update_time','create_time'],axis=1)
    df['statistics_date'] = df['statistics_date'].astype('str')
    column = df.columns.to_list()
    num = len(df['statistics_date'])
    h = 0
    while h < num:
        valve = df.loc[h].to_list()
        column_str = str(column).replace('[', "").replace(']', "").replace("'", "")
...
if __name__ == "__main__":
    # ShouYe()
    # pass_num_0(2,1,'330825_X735K5+800')
    # statistics_data()
    # t_bas_deal_case_statistics_data()
    # chushihuastation()
    # sql = {
    #     "tableName": "t_bas_basic_data_pass",
    #     "where": " statistic_date>= '2023-11-01' and statistic_date<= '2023-11-23' ",
    #     "columns": "*"
    # }
    # delete_data(sql)
    # 区县()

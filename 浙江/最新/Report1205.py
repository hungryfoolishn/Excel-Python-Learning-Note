...
class Reports:
    ...
    file_name = r"F:\Learn\工作\数据明细\静态表\1030test.xlsx"
    ...
    def __init__(self):
        ...
        out_file_name = r'F:\Learn\工作\数据明细\月报\{}月通报数据{}v1.0.xlsx'.format(this_month, today2)
        q案件 = '2023-01-01'
        s案件 = endtime
    ...
    def data_station1(self):
        ...
        ##报修站点
        maintain_station = {
            "tableName": " t_sys_station_maintain a inner join t_sys_station b on a.station_code=b.station_code and b.station_status in (0,3)  and a.insert_time between'2019-01-01 00:00:00'  and '{} 00:00:00'     ".format(
                self.end_time),
    ...
    def 总重80吨以上数据分析(self):
        ...
        with pd.ExcelWriter(r'D:\hungry\通报文件\总重80吨以上明细2.xlsx') as writer1:
    ...
    def 总重80吨以上明细(self):
        ...
        总重80吨以上明细 = df_big.sort_values(by=['area_county'],
                                       ascending=True).reset_index(drop=True)
        with pd.ExcelWriter(r'D:\hungry\通报文件\月报\总重80吨以上明细.xlsx') as writer1:
            总重80吨以上明细.to_excel(writer1, sheet_name='明细', index=False)
    ...
    def 超限100明细(self):
        ...
        with pd.ExcelWriter(r'D:\hungry\通报文件\月报\超限100明细.xlsx') as writer1:
    ...
    def 百吨王明细(self):
        ...
        with pd.ExcelWriter(r'D:\hungry\通报文件\月报\百吨王明细.xlsx') as writer1:
            U_过车_站点表.to_excel(writer1, sheet_name='百吨王明细', index=False)

    def Compliance_rate(self):
        df_big = pd.read_excel(r'D:\hungry\通报文件\月报\总重80吨以上明细.xlsx', sheet_name='明细')
        sql = {
    ...
    def over_100_num(self):
        ...
        U_过车_站点表 = pd.read_excel(r'D:\hungry\通报文件\月报\超限100明细.xlsx', sheet_name='超限100明细')
    ...
    def total_weight_100_num(self):
        U_过车_站点表 = pd.read_excel(r'D:\hungry\通报文件\月报\百吨王明细.xlsx', sheet_name='百吨王明细')
    ...
    def Key_freight_sources(self):
        ...
        站点总数 = pd.read_excel(r"D:\hungry\通报文件\静态表\重点货运源头445家明细0309.xlsx")
        ...
        t_bas_pass_data_71 = {
            ...
            "where": " out_station_time between '{} 00:00:00' and '{} 00:00:00'  and is_truck =1 and insert_time <='{} 00:00:00'".format(
    ...
    def city_excel(self):
        ...
        for city in city_list:
            ...
            with pd.ExcelWriter(r'D:\hungry\通报文件\记录表\{}{}月通报数据{}v1.0.xlsx'.format(city, self.this_month, self.today2)) as writer1:
            ...
            self.write_excel_file2(r'D:\hungry\通报文件\记录表\{}{}月通报数据{}v1.0.xlsx'.format(city, self.this_month, self.today2))
            city_path = r'D:\hungry\通报文件\记录表\{}{}月通报数据{}v1.0.xlsx'.format(city, self.this_month, self.today2)

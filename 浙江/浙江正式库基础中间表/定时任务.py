...
def insert_time():
    ...
    sql = "select  area_city,area_county,out_station,out_station_time,insert_time,car_no,total_weight,limit_weight from t_bas_pass_data_31 where insert_time >= '{} 15:00:00' and insert_time < '{} 00:00:00' ".format(
        starttime, endtime)
    t_bas_pass_data_31 = get_df_from_db(sql)

    # and area_county = 330781

    sql = "select  area_city,area_county,out_station,out_station_time,insert_time,car_no,total_weight,limit_weight from t_bas_pass_data_71 where insert_time >= '{} 15:00:00' and insert_time < '{} 00:00:00'  ".format(
        starttime, endtime)
    ...
    t_bas_pass_data_31['out_station_time'] = t_bas_pass_data_31['out_station_time'].map(lambda x: x.strftime('%Y-%m-%d'))
    t_bas_pass_data_31['insert_time'] = t_bas_pass_data_31['insert_time'].map(lambda x: x.strftime('%Y-%m-%d'))
    ...
    U_pass31_area_station['overrun_rate'] = (U_pass31_area_station['total_weight'] - U_pass31_area_station['limit_weight']) * 100/U_pass31_area_station['limit_weight']

    # U_pass31_area_station['car_no'] = U_pass31_area_station['car_no'].apply(lambda car_no: car_no[:2])


    t_bas_pass_data_71['out_station_time'] = t_bas_pass_data_71['out_station_time'].map(lambda x: x.strftime('%Y-%m-%d'))
    t_bas_pass_data_71['insert_time'] = t_bas_pass_data_71['insert_time'].map(lambda x: x.strftime('%Y-%m-%d'))
    ...
    T0_5T = U_pass31_area_station[
        ...
        ['out_station','out_station_time', 'insert_time'])['car_no'].count().reset_index(name='T0_5T')
    ...
    over_num = U_pass31_area_station[(U_pass31_area_station['overrun_rate'] > 0)].groupby(['out_station','out_station_time', 'insert_time'])[
    ...
    over_num = U_pass71_area_station[(U_pass71_area_station['overrun_rate'] > 0)].groupby(['out_station','out_station_time', 'insert_time'])[
        'car_no'].count().reset_index(name='over_num')
    ...
    wide_table['id'] = wide_table['station_code'].astype('string') + wide_table['statistics_date'].astype('string')+wide_table['insert_time'].astype('string')
    ...
    # 连接数据库,定义变量
    db = pymysql.connect(host='192.168.2.39', user='zjzhzcuser', password='F4dus0ee', port=3306,
...
schedule.every(10).seconds.do(insert_time)
while True:

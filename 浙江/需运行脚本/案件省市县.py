...
while i!=I:
            ...
            sql = "SELECT * FROM t_bas_over_data_collection_makecopy where insert_time >='{} 00:00:00' and insert_time <'{} 00:00:00' ".format(q,s)
            ...
            T_入库数 = T_入库查询.groupby(['area_city','city','area_county','county'])['id_x'].count().reset_index(name='入库数(系统)')

            T_现场处罚 = T_现场处罚.drop_duplicates(['CASE_NUM'])
            T_现场处罚计数 = T_现场处罚.groupby(['area_city','city','area_county','county'])['CASE_NUM'].count().reset_index(name='现场处罚(系统)')


            T_非现场处罚 = T_非现场处罚.drop_duplicates(['CASE_NUM'])
            T_非现场处罚计数 = T_非现场处罚.groupby(['area_city','city','area_county','county'])['CASE_NUM'].count().reset_index(name='非现场处罚(系统)')


            T_外省抄告计数= T_外省抄告.groupby(['area_city','city','area_county','county'])['id_x'].count().reset_index(name='外省抄告(系统)')
            ...
            W_案件统计['非现场处罚率(含抄告)']=((W_案件统计['非现场处罚(系统)']+W_案件统计['外省抄告(系统)']) / (W_案件统计['入库数(系统)']+0.00001)* 100).round(2)
            ...
            W_案件统计.rename(
                columns={'入库数(系统)': 'in_case_num',
                         ...
                         '案件数': 'total_case_num', '非现场处罚率(不含抄告)': 'p_offsite_rate', '非现场处罚率(含抄告)': 'p_offsite_inform_rate'}, inplace=True)
            ...
            W_案件统计地市['p_offsite_inform_rate']=((W_案件统计地市['p_offsite_num']+W_案件统计地市['f_inform_num']) / (W_案件统计地市['in_case_num']+0.00001)* 100).round(2)
            ...
            W_案件统计省级['p_offsite_inform_rate']=((W_案件统计省级['p_offsite_num']+W_案件统计省级['f_inform_num']) / (W_案件统计省级['in_case_num']+0.00001)* 100).round(2)

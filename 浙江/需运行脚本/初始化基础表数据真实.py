...
while c>=0:
            ...
            sql = "SELECT out_station,count(1) as pass_num,sum(is_truck) as truck_num FROM t_bas_pass_data_31_20230501 WHERE out_station_time >= '{} 00:00:00' AND out_station_time < '{} 00:00:00' GROUP BY out_station".format(
                starttime, endtime)
            pass_truck_num = get_df_from_db(sql)
            pass_truck_num = pd.DataFrame(pass_truck_num)
            print(pass_truck_num)
            # pass_truck_num.to_excel(r'C:\Users\liu.wenjie\Desktop\月报\10月\pass_truck_num.xlsx')


            sql = "select  area_city,area_county,out_station,out_station_time,car_no,total_weight,limit_weight ,overrun_rate from t_bas_over_data_31 where out_station_time >= '{} 00:00:00' and out_station_time < '{} 00:00:00' AND allow IS NULL  ".format(
                starttime, endtime)
            ...
            #     starttime, endtime)
            # t_bas_pass_data_71 = get_df_from_db(sql)
            sql1 = "select  station_code,station_status,station_type,is_check_station,station_name, area_county from t_sys_station where is_deleted = 0   and station_type in (31,71) "

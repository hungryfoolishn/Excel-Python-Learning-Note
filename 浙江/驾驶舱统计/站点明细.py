...
区县超限100明细= pd.read_excel(r"G:\智诚\日常给出数据汇总\月通报\9月统计\月报\结果表\9月驾驶舱数据\t_sys_station1015.xlsx")
...
区县超限100数=区县超限100明细.groupby([区县超限100明细['区县']])['序号'].count().reset_index(name='超限100数')
# 最终= pd.merge(区县超限100数,地市编码表,left_on=['区划代码'],right_on=['county_code'],how='left')

print(区县超限100数)
区县超限100数.to_excel(r"G:\智诚\日常给出数据汇总\月通报\9月统计\月报\结果表\9月驾驶舱数据\超限100%数.xlsx")
# print(最终.info())

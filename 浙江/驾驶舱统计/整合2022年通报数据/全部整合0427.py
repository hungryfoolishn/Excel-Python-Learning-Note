...
def data_station():
    ...
    ##报修站点
    sql = "SELECT b.station_name,a.station_code,reason as '备注' FROM t_sys_station_maintain a inner join t_sys_station b on a.station_code=b.station_code and b.station_status in (0,3) where   end_time>='{}'   or end_time is null and a.insert_time <'{}'".format(starttime, endtime)
    ...
    df = pd.DataFrame(df, columns=['站点名称', '地市', '区县', '理应在线天数', '实际在线天数', '在线率', '货车数', '超限数', '百吨王数', '超限100%数',
                                   ...
                                   'station_code'])
    df_数据汇总 = df.sort_values('county_code', ascending=True)
...
if __name__ == "__main__":
    ...
    with pd.ExcelWriter(r'C:\Users\liu.wenjie\Desktop\月报\202304\test.xlsx') as writer1:

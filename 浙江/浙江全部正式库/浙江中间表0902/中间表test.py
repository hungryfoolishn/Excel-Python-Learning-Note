...
while i!=I:
    ...
    #
    #
    # print('暂停：', sleeptime(12, 0, 20), '秒')
    # seconds = sleeptime(12, 0, 20)
    # time.sleep(seconds)
    from datetime import datetime
    ...
    sql = "select  area_city,area_county,out_station,out_station_time,car_no,total_weight,limit_weight from t_bas_pass_data_31 where out_station_time >= '{} 00:00:00' and out_station_time < '{} 00:00:00'  and area_county=330781 ".format(
        starttime, endtime)
    t_bas_pass_data_31 = get_df_from_db(sql)
    print(t_bas_pass_data_31)
    # sql = "select out_station,out_station_time,car_no,total_weight,limit_weight,overrun,overrun_rate from t_bas_pass_data_highspeed where out_station_time >= '{} 00:00:00' and out_station_time < '{} 00:00:00' ".format(
    #     starttime, endtime)
    # t_bas_pass_data_41 = get_df_from_db(sql)
    ...
    sql1 = "select  station_code,station_status,station_type,station_name, area_county from t_sys_station where is_deleted = 0  and station_status=0 and station_type=31 and area_county=330781 "
    ...
    # U_pass41_area_station = t_bas_pass_data_41
    # U_pass41_area_station['car_no'] = U_pass41_area_station['car_no'].fillna(value=0)
    ...
    truck_num = U_pass31_area_station.groupby(['out_station'])['car_no'].count().reset_index(
        name='truck_num')
    wide_table_31 = pd.merge(wide_table_31, truck_num, on=['out_station'], how='left')
    ...
    # table_name = 't_bas_basic_data_pass'
    # keys = wide_table.keys()
    # values = wide_table.values.tolist()
    #
    # key_sql = ','.join(keys)
    # value_sql = ','.join(['%s'] * wide_table.shape[1])
    #
    # # 插入语句
    ...
    # # 提交数据库操作
    # try:
    #     mycursor.executemany(insert_data_str, values)
    #     db.commit()
    ...
    #     #result_table
    #     #要写入的数据表，这样写的话要提前在数据库建好表
    #     wide_table.to_sql(name='t_bas_basic_data_pass', con=engine, if_exists='append', index=False)
    # except Exception as e:
    #             print("mysql插入失败")

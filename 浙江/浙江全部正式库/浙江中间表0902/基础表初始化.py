...
while c>=5:
    ...
    sql = "select  area_city,area_county,out_station,out_station_time,car_no,total_weight,limit_weight from t_bas_pass_data_31_20221101 where out_station_time >= '{} 00:00:00' and out_station_time < '{} 00:00:00' ".format(
        starttime, endtime)
    ...
    #     starttime, endtime)
    # t_bas_pass_data_71 = get_df_from_db(sql)
    sql = "select  station_code,station_status,station_type,station_name, area_county from t_sys_station where is_deleted = 0  and station_status=0 and station_type in (31,71) "
    t_sys_station = get_df_from_db(sql)
    sql = "select city_code,county_code,city,county FROM t_code_area"
    ...
    mycursor = db.cursor()
    sql = "DELETE FROM t_bas_basic_data_pass WHERE statistic_date = '{}'".format(starttime)

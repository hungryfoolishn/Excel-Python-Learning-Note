...
while i!=I:
    ...
    U_pass31_area_station['overrun_rate'] = U_pass31_area_station.apply(lambda x: (x['total_weight'] - x['limit_weight']) * 100/ x['limit_weight'], axis=1).round(4)
    ...
    db = pymysql.connect(host='192.168.2.39', user='zjzhzcuser', password='F4dus0ee', port=3306,
                         db='db_manage_overruns')
    cursor = db.cursor()
    table = "t_bas_basic_data_pass"  # 写入表名

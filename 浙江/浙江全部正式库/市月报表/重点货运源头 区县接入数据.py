...
sql = "SELECT * from t_bas_source_company where area_city = '330600' and is_statictis =1  and is_deleted=0 "
...
q = input("请输入储存路径(C:/Users/Administrator/Desktop/输出报表/月报表相关数据汇总)：")
with pd.ExcelWriter('{}/站点完好数.xlsx'.format(q))as writer1:
...
q = input("请输入储存路径(C:/Users/Administrator/Desktop/输出报表/月报表相关数据汇总)：")
with pd.ExcelWriter('{}/源头.xlsx'.format(q))as writer1:

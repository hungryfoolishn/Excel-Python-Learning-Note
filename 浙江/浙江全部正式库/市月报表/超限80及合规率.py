...
sql = "SELECT * FROM t_bas_over_data_31 where out_station_time >= '2022-10-01 00:00:00' and  out_station_time <'2022-11-01 00:00:00' and overrun_rate>80  AND allow IS NULL AND is_unusual = 0  "
t_bas_over_data_31=get_df_from_db(sql)

sql = "SELECT * FROM t_bas_over_data_collection_31 where out_station_time >= '2022-10-01 00:00:00' and out_station_time <'2022-11-01 00:00:00'  and overrun_rate>80  "
...
if cs=='浙江':
    ...
    超限80数 = pd.merge(月超限80以上总数, 月超限80以上且满足处罚条件总数, on='city', how='outer')

    超限80数 = pd.merge(超限80数, 月超限80以上审核通过总数, on='city', how='outer')

    超限80数.rename(columns={'id_x_x': '月超限80%以上总数', 'id_x_y': '月超限80%以上且满足处罚条件总数', 'id_x': '月超限80%以上审核通过总数'},
                 inplace=True)
    print(超限80数)
    超限80数.to_excel(r"G:\智诚\日常给出数据汇总\月通报\10月统计\结果表\月报\2浙江省超限80%以上数据.xlsx")
        ...
        print(超限80数)
        超限80数.to_excel(r"G:\智诚\日常给出数据汇总\月通报\10月统计\结果表\月报\全部区县超限80%以上数据.xlsx")

...
def tjfx():
    ...
    try:
        ...
        df_数据汇总 = set_columns_date_frame(df_数据汇总, 2, 0)
        print(df_数据汇总)
        ...
        if cs == '浙江':
            ...
            T_20天与500筛选 = pd.DataFrame(T_20天与500筛选,
                                       columns=["站点名称", "地市", "区县", "理应在线天数", "实际在线天数", "在线率", "货车数", "超限20%除外数",
                                                "超限10%除外超限率(%)"])
            T_20天与500筛选.rename(columns={'理应在线天数': '应在线天数', '超限20%除外数': '超限数', '超限10%除外超限率(%)': '超限率'}, inplace=True)
            ...
            超限数 = df_数据汇总.groupby(['区县'])['超限20%除外数'].sum()
            ...
            实际站点数 = pd.merge(在用数, 报修数, on='区县', how='outer')
            实际站点数 = 实际站点数.fillna(0, inplace=False)
            ...
            T_500筛选.columns = ['站点名称']
            T_500筛选.rename(columns={'站点名称': '500辆次'}, inplace=True)
            T_筛选 = pd.merge(T_10筛选, T_500筛选, how='outer', on='区县')
            T_筛选 = T_筛选.fillna(value=0)
            T_筛选['在线天数＜10天或货车数＜500辆次'] = T_筛选['站点名称'] + T_筛选['500辆次']
            T_10或500 = T_筛选['在线天数＜10天或货车数＜500辆次']
            t = df_数据汇总[(df_数据汇总.实际在线天数 == 0)]
            ...
            with pd.ExcelWriter('{}/杭州市公路治超超限率以及非现场执法点设施表.xlsx'.format(b1)) as writer1:
            ...
            湖州附件1['超限率'] = 湖州附件1.apply(lambda x: x['超限20%除外数'] / x['货车数'], axis=1).round(4)
            ...
            湖州附件1.rename(columns={'超限20%除外数': '超限数'}, inplace=True)
            #print(湖州附件1)
            ...
            with pd.ExcelWriter('{}/湖州市普通公路超限率以及非现场执法点位设备完好率.xlsx'.format(b1)) as writer1:
            ...
            """附件1"""
            货车数 = df_数据汇总.groupby(['地市','区县'])['货车数'].sum()
            超限数 = df_数据汇总.groupby(['地市','区县'])['超限20%除外数'].sum()
            ...
            with pd.ExcelWriter('{}/{}市普通公路超限率以及非现场执法点位设备完好率表.xlsx'.format(b1, cs)) as writer1:

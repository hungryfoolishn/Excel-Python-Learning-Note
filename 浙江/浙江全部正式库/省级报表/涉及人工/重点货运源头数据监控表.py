...
sql = "SELECT * FROM t_bas_pass_data_71  where out_station_time >='2022-08-01' and out_station_time <='2022-09-01' and area_province  = '330000' and is_truck =1 and insert_time <='2022-09-07' "
t_bas_pass_data_71 =get_df_from_db(sql)
站点总数 = pd.read_excel("C:/Users/Administrator/Desktop/系统拉出表/省级月报/重点货运源头接入数模板.xlsx")
站点总数['city_code1']=站点总数['city_code1'].apply(str)
print(站点总数)
"""拼接表"""
...
企业_源头_站点表.to_excel("C:/Users/Administrator/Desktop/企业_源头_站点表.xlsx")
...
超限20_50 = U_源头_区域表[(U_源头_区域表['overrun_rate'] > 20) &(U_源头_区域表['overrun_rate'] <= 50)].groupby(['city_code','city'])['car_no'].count()
...
在线天数大于20天=U_源头_区域表.groupby(['city_code','city','out_station'])['取日'].nunique()
在线天数大于20天 = 在线天数大于20天.to_frame()
在线天数大于20天= 在线天数大于20天.reset_index()
货运量大于2万吨=U_源头_区域表.groupby(['city_code','city','out_station'])['total_weight'].sum()
货运量大于2万吨 =货运量大于2万吨.to_frame()
货运量大于2万吨= 货运量大于2万吨.reset_index()
过车数大于410辆=U_源头_区域表.groupby(['city_code','city','out_station'])['id_x'].count()
...
各市站点数=站点完好数[(站点完好数['取日'] > 20) | (站点完好数['total_weight'] > 20000) | (站点完好数['id_x'] > 410)].groupby(['city_code','city'])['out_station'].count()
...
货运源头监控数据=pd.merge(货运源头监控数据,数据站点总数,left_on='city_code1',right_on='area_city_y',how='left')
...
货运源头监控数据['源头单位平均过车数（辆次）'] = 货运源头监控数据.apply(lambda x: x['货车数']/(x['数据站点总数']+0.0001) , axis=1).round(2)
货运源头监控数据['设备上线率（%）'] = 货运源头监控数据.apply(lambda x: x['各市站点数']/(x['数据站点总数']+0.0001), axis=1).round(2)
货运源头监控数据 =货运源头监控数据.fillna(0, inplace=False)
...
with pd.ExcelWriter('{}/地市源头.xlsx'.format(q))as writer1:

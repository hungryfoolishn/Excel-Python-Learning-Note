...
sql = "SELECT * FROM t_bas_over_data_31 where out_station_time >= '2022-05-01 00:00:00' and  out_station_time <'2022-06-01 00:00:00' and overrun_rate>80  AND allow IS NULL AND is_unusual = 0  "
t_bas_over_data_31=get_df_from_db(sql)

sql = "SELECT * FROM t_bas_over_data_collection_31 where out_station_time >= '2022-05-01 00:00:00' and  out_station_time <'2022-06-01 00:00:00' and overrun_rate>80  and update_time <'2022-06-06 15:10:00'"
...
U_案件审核_区域表 = pd.merge(t_bas_over_data_collection_31,t_code_area,left_on='area_county',right_on='county_code',how='left')
...
超限80数.to_excel("C:/Users/Administrator/Desktop/输出报表/2022年处罚案件统计情况/超限80数.xlsx")
"""U_过车_站点表=U_过车_站点表[(U_过车_站点表['is_collect']==1)
                        ]
print(U_过车_站点表)
月超限80以上且满足处罚条件总数=U_过车_站点表.groupby(['city_code','city']).count()
